        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {templates.map(template => (
            ...
            >
              ...
              <div className="flex items-center gap-2 mt-4 pt-4 border-t border-[#EAEAEA]">
                <button 
                  onClick={(e) => { e.stopPropagation(); setSelectedTemplate(template); }}
                  className="flex-1 py-1.5 text-[12px] font-semibold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 rounded-lg transition-colors border border-indigo-100"
                >
                  Preview Workflow
                </button>
                <button 
                  onClick={(e) => { e.stopPropagation(); }}
                  className="flex-1 py-1.5 text-[12px] font-semibold text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition-colors shadow-sm border border-indigo-700"
                >
                  Apply Template
                </button>
              </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      ...
      >
        ...
        <div className="flex-1 overflow-hidden flex flex-col md:flex-row bg-gray-50/30">
          ...
          <div className="flex-1 overflow-y-auto flex flex-col bg-gray-50/10">
            <div className="sticky top-0 bg-white/95 backdrop-blur-sm z-20 border-b border-[#EAEAEA] px-8 pt-8">
              ...
              <div className="flex space-x-6">
                <button
                  ...
                  className={`pb-3 text-[13px] font-bold transition-colors relative ${activeTab === 'timeline' ? 'text-indigo-600' : 'text-gray-500 hover:text-gray-900'}`}
                >
                  Timeline View
                  {activeTab === 'timeline' && <motion.div layoutId="modalTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-indigo-600" />}
                </button>
                <button
                  onClick={() => setActiveTab('dependency')}
                  className={`pb-3 text-[13px] font-bold transition-colors relative ${activeTab === 'dependency' ? 'text-indigo-600' : 'text-gray-500 hover:text-gray-900'}`}
                >
                  Dependency View
                  {activeTab === 'dependency' && <motion.div layoutId="modalTab" className="absolute bottom-0 left-0 right-0 h-0.5 bg-indigo-600" />}
                </button>
              </div>
            </div>
