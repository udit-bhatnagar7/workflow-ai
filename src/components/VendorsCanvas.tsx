    ...
    >
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {vendor.categories.map((cat: string) => (
          <span key={cat} className="text-[11px] font-medium text-gray-600 bg-gray-100 px-2 py-0.5 rounded">
            {cat}
          </span>
        ))}
        {vendor.badge && (
          <span className="text-[11px] font-medium text-indigo-700 bg-indigo-50 border border-indigo-100 px-2 py-0.5 rounded flex items-center">
            <Sparkles className="w-3 h-3 mr-1" /> {vendor.badge}
          </span>
        )}
      </div>
      
      {/* Performance Row */}
      <div className="flex items-center text-[12px] text-gray-500 mb-5 bg-[#FAFAFA] border border-[#F0F0F0] rounded-md px-3 py-2">
      ...
      <div className="mb-6 flex-1">
        <div className="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-2 flex items-center">
          <CalendarDays className="w-3.5 h-3.5 mr-1.5" /> Next Available: <span className="text-gray-900 ml-1">{vendor.nextAvailable}</span>
        </div>
        <div className="flex space-x-2">
          {vendor.availability.map((day: any, idx: number) => (
            <div key={idx} className={`flex items-center text-[11px] px-2 py-1 rounded border ${day.status === 'available' ? 'bg-emerald-50 border-emerald-100 text-emerald-700' : 'bg-gray-50 border-gray-200 text-gray-400'}`}>
              {day.date} 
      ...
      <div className="flex items-center space-x-2 mt-auto pt-4 border-t border-[#EAEAEA]">
        <button 
          onClick={onOpenModal}
          className="flex-1 px-3 py-2 text-[13px] font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-md shadow-sm transition-colors"
        >
          Schedule Vendor
        </button>
        <button className="px-3 py-2 text-[13px] font-medium text-gray-700 bg-white border border-[#EAEAEA] hover:bg-gray-50 rounded-md transition-colors">
          Profile
        </button>
        <div className="flex space-x-1">
          <button className="p-2 text-gray-500 hover:text-gray-700 hover:bg-gray-100 rounded-md transition-colors border border-transparent hover:border-[#EAEAEA]">
...
function ScheduleModal({ vendor, onClose }: { vendor: any, onClose: () => void }) {
  const [selectedDate, setSelectedDate] = useState('Mar 5');
              ...
              {['Photographer', 'Drone', 'Cleaner', 'Stager', 'Inspector', 'Handyman', 'Landscaper'].map(cat => {
                ...
                return (
                  <button 
                    ...
                    className={`px-3 py-1.5 text-[12px] font-medium rounded-md transition-all border ${
                      isSelected 
